...
import { startCheckout } from "@/lib/woo.functions";
  ...
  async function onCheckout() {
    ...
    try {
      const res = await checkoutFn({
        data: { items: items.map((i) => ({ sku: i.slug, quantity: i.qty })) },
      });
      if (res.ok) {
      ...
      <SheetContent className="bg-brand-forest-deep border-white/10 text-foreground flex flex-col w-full sm:max-w-md p-0">
        ...
        {items.length === 0 ? (
          ...
          <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
            {items.map((item) => (
              <div key={item.slug} className="flex gap-4 pb-4 border-b border-white/5">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    <button
                      ...
                      className="w-7 h-7 rounded-full border border-white/10 flex items-center justify-center text-foreground/70 hover:text-foreground hover:border-white/30"
                      aria-label="Decrease"
                    >
                    ...
                    <button
                      ...
                      className="ml-auto text-foreground/40 hover:text-red-400"
                      aria-label="Remove"
                    >
